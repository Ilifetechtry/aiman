...
if(isset($_POST['save'])){   
    ...
    $select1 = mysqli_query($conn, "SELECT * FROM  credit where bill_id='" . $_GET['id'] . "'");
    $fc = mysqli_fetch_assoc($select1);
    $c=$_POST['due_amount']-$_POST['paid_amount'];
    $upd_customer2=mysqli_query($conn,"UPDATE credit set cname='".$_POST['cname']."',bill_id='".$_GET['id']."',paid_amount='".$_POST['paid_amount']."',balance_amount='".$_POST['balance_amount']."',date='".$_POST['date']."' where bill_id='".$_GET['id']."'");
    $upd_customer21=mysqli_query($conn,"INSERT INTO credit1(cname,bill_id,paid_amount,balance_amount,date,due_amount)VALUES('".$_POST['cname']."','".$_GET['id']."','".$_POST['paid_amount']."','".$_POST['balance_amount']."','".$_POST['date']."','" . $fc['balance_amount'] . "')");

    $select_cus_rept=mysqli_query($conn,"SELECT * FROM credit where bill_id='".$_GET['id']."'");    
    ...
    $selc=mysqli_query($conn,"SELECT * FROM credit where balance_amount=0");
    if(mysqli_num_rows($selc)>0){
        $fe=mysqli_fetch_assoc($selc);
        // $del=mysqli_query($conn,"UPDATE credit SET balance_amoun="Paid" where bill_id='".$fe['bill_id']."'");
        $del=mysqli_query($conn,"UPDATE customer1 set c_amount='' ,r_amount='' ,credit_status=0 where id='".$fe['bill_id']."'");
    }
    ?>
...
<div class="col-lg-4 new_cus">
<div class="form-group"><label for="name" class="form-label">Customer Name<spam>*</spam></label>
<div class="form-control-wrap">
<input type="text" class="form-control" name="cname" id="cname" placeholder="Enter your name" value="<?=$fc['cname'];?>" required readonly/>
</div></div></div>
<div class="col-lg-4">
<div class="form-group"><label for="date" class="form-label">Date<spam>*</spam></label>
<div class="form-control-wrap">
<input type="date" class="form-control" id="date"  name="date" value="<?=date('Y-m-d');?>" readonly></div></div></div>

    <div class="col-lg-4">
<div class="form-group"><label for="net_amount" class="form-label">Due Amount<spam>*</spam></label>
<div class="form-control-wrap">
    <input type="text" class="form-control" id="main_total" name="due_amount" value="<?=$fc['balance_amount'];?>" required readonly></div></div></div>
