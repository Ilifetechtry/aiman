...
if(isset($_POST['submit'])){
    $sel1=mysqli_query($conn,"SELECT * FROM district where district='".$_POST['district']."'");
    if(mysqli_num_rows($sel1)>0){
        ...
        $ins=mysqli_query($conn,"INSERT INTO district (district) values ('".$_POST['district']."')");
        ?>
        <script>
                Swal.fire({
                    icon: 'success',
...
<div class="nk-block-head">
    <div class="nk-block-head-between flex-wrap gap g-2">
        <div class="nk-block-head-content">
            <h2 class="nk-block-title">City</h2>
            <nav>
...
<form method="post">
        <div class="col-12">
            ...
            <div class="col-3">
                <input  name="district" type="text" class="form-control" placeholder="Enter Place" style="border:1px solid silver" required>
            </div>
            <div class="col-2">
                <button class="btn btn-primary w-100" name='submit'>
                    <i class='ni ni-plus'></i>Add City
                </button>
            </div>
            </div>
...
<tbody>
<?php      
    $sel=mysqli_query($conn,"SELECT * FROM district order by id desc");
    $i=0;
...
<td class="tb-col tb-col-md"><span><?=$fe['district']?></span></td>
...
<script>
    let delete_url='delete-district.php?id=';
</script>
<?php
